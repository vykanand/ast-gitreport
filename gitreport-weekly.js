...
// Function to get the previous 2 weeks date range
function getDateRange(weeksBack = 2) {
    const now = new Date();
    const start = new Date(now);
    start.setDate(now.getDate() - (weeksBack * 7)); // 2 weeks back
...
async function generateHTMLReport(repoMetrics, monthRange) {
    ...
    const filePath = path.join(__dirname, `multirepo-report-biweekly.html`);
